...
describe('Testing the creation and methods of the Engineer object', () => {
    test('Creates an engineer object', () => { // Setting up the test.
        const engineer = new Engineer('Name', 3, "GetHub"); //Creating the object that we want to test.
        expect(engineer.name).toBe('Name'); //Things we want to test.
        expect(engineer.id).toEqual(expect.any(Number));
        expect(engineer.github).toEqual(expect.any(String));
        expect(engineer.email).toBe("Name@work.ca");
    });

    test('Gets engineer\'s name', () => {
        const engineer = new Engineer('Name', 3, "GetHub");
    ...
    test('gets engineer\'s email', () => {
        const engineer = new Engineer('Name', 3, "GetHub");
        expect(engineer.getEmail()).toEqual('Name@work.ca');
    });
